...
import { FileText, Calculator, Code, FlaskConical, Info, Book, Database, Thermometer, Settings, Activity } from "lucide-react";
      ...
      <main className="flex-1 py-16 px-6">
        <div className="max-w-screen-xl mx-auto">
          <div className="mb-12 flex justify-between items-center bg-blue-50 rounded-xl p-6">
            ...
            <Button variant="outline" className="flex gap-2 items-center border-blue-300 text-blue-800 hover:bg-blue-100" onClick={() => setIsLearnMoreOpen(true)}>
          ...
          <div className="mb-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {coreDashboardCards.map(card => <Link key={card.title} to={card.linkTo} className="block rounded-xl border border-blue-100 p-6 hover:bg-blue-50 transition-all duration-300 group shadow-sm hover:shadow-md py-[21px]">
          ...
          <div className="mb-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
              {softwareTypeCards.map(card => <Link key={card.title} to={`/software-tools?type=${encodeURIComponent(card.type)}`} className="block rounded-xl border border-blue-100 p-4 hover:bg-blue-50 transition-all duration-300 group shadow-sm hover:shadow-md">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {utilityDashboardCards.map(card => <Link key={card.title} to={card.linkTo} className="block rounded-xl border border-blue-100 p-6 hover:bg-blue-50 transition-all duration-300 group shadow-sm hover:shadow-md">
                  <div className="flex justify-between items-start mb-4">
                    <div className={`p-3 rounded-xl ${card.color} bg-opacity-10 group-hover:scale-105 transition-transform`}>
                      {card.icon}
                    </div>
